...
public class SendHandler implements Runnable{
    ...
    int dstPort;
    int startSEQ;
    int rwind = 5 * Utils.MAX_SEG_SIZE;   //流量控制窗口 (初始化为5MSS, 但其实应该是握手的时候交换这个数据的初始值)
    int cwind = Utils.MAX_SEG_SIZE;       //阻塞控制窗口, 慢启动(初始值为1MSS)
    int last_cwind = 102400;        //一开始将last_cwind设置很大, 以便于快速启动
    Boolean end = false;
    
    public SendHandler(DatagramSocket socket, String filename, InetAddress dstAddr, int dstPort, int seq){
        this.socket = socket;
        this.filename = filename;
        this.dstAddr = dstAddr;
        this.dstPort = dstPort;
        this.startSEQ = seq;
        packet = new DatagramPacket(new byte[Utils.MAX_SEG_SIZE+64], Utils.MAX_SEG_SIZE+64); //还有header
    }
    ...
    public void run(){
        try{
            ...
            while ((bufferLength = fis.read(bs)) != -1 && !end) {
                ...
                System.out.println("Sender: 刷新缓冲区");
                // 开始将这一条缓冲区里的内容全部发出并ACK
                while(bfpt_waiting_seq != bufferLength){
                	Thread.sleep(50);
                	// 检查是否收到了新ACK,并作出相应处理
                    ...
                    if(!thread_timer.isAlive()) {
                    	myTimer.reset();
                    	thread_timer = new Thread(myTimer);
                    	thread_timer.start();
                    }
